...
export function SignIn({ onSuccess, onSwitchToSignUp, onBack }: SignInProps) {
  ...
  const handleDemoLogin = async () => {
    setEmail('demo@ecosync.com');
    setPassword('demo123');
    setError('');
    setIsLoading(true);

    try {
      const response = await signIn('demo@ecosync.com', 'demo123');
      
      if (response.success) {
        setSuccess('Welcome to EcoSync Demo!');
        setTimeout(() => {
          onSuccess();
        }, 1000);
      } else {
      ...
      >
        ...
        <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-8 shadow-2xl border border-white/20">
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full bg-white/10 border border-white/20 text-white py-3 rounded-xl font-semibold hover:bg-white/20 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
          >
        ...
        <div className="mt-4 p-4 bg-white/5 backdrop-blur rounded-xl border border-white/10">
          <p className="text-white/60 text-sm text-center">
            <span className="text-green-400 font-semibold">Demo credentials:</span><br />
            Email: demo@ecosync.com | Password: demo123
          </p>
        </div>
      </motion.div>
    </div>
